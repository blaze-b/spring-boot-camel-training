package com.mclebtec.demo.order;

import com.mclebtec.demo.order.exception.OrderException;
import com.mclebtec.demo.order.exception.OrderValidationException;
...
public class OrderService {
    ...
    public void toSoap(Exchange exchange) {
        ...
        if (body.contains("ActiveMQ in Action")) {
            ...
            InputStream is = exchange.getContext().getClassResolver().loadResourceAsStream("exceptions/soapFault.xml");
            ...
            InputStream is = exchange.getContext().getClassResolver().loadResourceAsStream("exceptions/soapOk.xml");
            Document dom = exchange.getContext().getTypeConverter().convertTo(Document.class, is);
            // set a xml reply
            exchange.getOut().setBody(dom);
        }
